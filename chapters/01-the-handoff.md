...
The output of a build is an **artifact**: a file or a package that can be handed to a machine and run — and once it is running it is said to be at **runtime**, the state of a program while it is actually executing rather than sitting on disk as text, which is where most of the interesting failures in this book happen. And here is the part people find genuinely surprising the first time: *the artifact is derived, not authored*. You do not edit it directly, and you do not treat it as the source of truth — real systems often do keep artifacts around for a while, in a registry, for rollback or an audit or reproducing what was actually running last Tuesday, and that is fine. What matters is that keeping one is a convenience, not a responsibility: if you lose it, you should be able to reproduce an equivalent one from the source and the recorded build inputs. The recipe is the thing you protect. The meal is easy to make again.
